...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLogEnabled) {
            ...
            i(LOG_TAG, "Drawtime: " + drawtime + " ms, average: " + average + " ms, cycles: "
                    + drawCycles);
        }
    }
    ...
    protected void prepareValuePxMatrix() {
        if (mLogEnabled)
            i(LOG_TAG, "Preparing Value-Px Matrix, xmin: " + mXAxis.mAxisMinimum + ", xmax: "
    ...
    public void notifyDataSetChanged() {
        if (mData == null) {
            if (mLogEnabled)
                i(LOG_TAG, "Preparing... DATA NOT SET.");
            return;
        } else {
            if (mLogEnabled)
                i(LOG_TAG, "Preparing...");
    ...
    public void calculateOffsets() {
        if (!mCustomViewPortEnabled) {
            ...
            if (mLogEnabled) {
                ...
                i(LOG_TAG, "Content: " + mViewPortHandler.getContentRect().toString());
            }
        }
        prepareOffsetMatrix();
